...
// ipmi sel clear 부분 test를 위해 주석처리해둠
// 사전설정 테스트 해야됨
...
void dmesg_ERR()
{
    ...
    // char CPUTIME[MID_Byte];
    // char CPUTIME_TMP[MID_Byte];
    // fp = popen("grep -m1 \"\\.clock\" /proc/sched_debug | awk -F ':' '{print $2}' | sed 's/ //g' | awk -F '.' '{print $1}'","r");

    // fscanf(fp,"%s",CPUTIME_);
